# Encurtador de URL — Lua (LuaSocket)

API de encurtamento de URLs desenvolvida em **Lua**, utilizando PostgreSQL como banco de dados (via `pgmoon`) e servidor HTTP próprio com **LuaSocket**. Todas as rotas são protegidas por autenticação **Basic Auth**.

## Tecnologias

| Componente   | Tecnologia                              |
|--------------|-----------------------------------------|
| Runtime      | Lua 5.1+ / LuaJIT                       |
| Servidor HTTP| LuaSocket (TCP server)                  |
| Banco        | PostgreSQL (pgmoon)                     |
| Cache        | Tabela Lua (single-thread safe)         |
| Autenticação | Basic Auth (base64 decode em Lua puro)  |
| ID           | math.random + math.randomseed           |
| JSON         | dkjson                                  |
