# Encurtador de URL — Ruby on Rails

API de encurtamento de URLs desenvolvida em Ruby com **Ruby on Rails** (API-only), utilizando PostgreSQL como banco de dados e cache em memória com `Monitor` (Mutex re-entrante). Todas as rotas são protegidas por autenticação **Basic Auth**.
...
| Servidor     | Puma                                          |
| Banco        | PostgreSQL (`pg` gem)                         |
| Cache        | Hash + Monitor (thread-safe)                  |
| Autenticação | authenticate_or_request_with_http_basic       |
| ID           | SecureRandom (8 chars, a-zA-Z0-9)             |
...
rails db:migrate
rails server -b 0.0.0.0 -p $HTTP_PORT
```
