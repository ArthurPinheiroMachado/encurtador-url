# Encurtador de URL — .NET (ASP.NET Core)

API de encurtamento de URLs desenvolvida em C# com **ASP.NET Core** (Minimal APIs), utilizando PostgreSQL como banco de dados (Entity Framework Core + Npgsql) e cache em memória com `ConcurrentDictionary`. Todas as rotas são protegidas por autenticação **Basic Auth**.
...
| Banco        | PostgreSQL (Npgsql.EntityFrameworkCore)           |
| Cache        | ConcurrentDictionary (thread-safe)               |
| Autenticação | AuthenticationHandler custom                     |
| ID           | RandomNumberGenerator (8 chars, a-zA-Z0-9)       |
...
dotnet run --project Encurtador.Url/Encurtador.Url.csproj
```
