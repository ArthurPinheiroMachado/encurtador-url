...
API de encurtamento de URLs desenvolvida em Python com **FastAPI**, utilizando PostgreSQL como banco de dados (via `asyncpg`) e cache em memória com `asyncio.Lock`. Todas as rotas são protegidas por autenticação **Basic Auth**.
...
| Cache        | dict + asyncio.Lock                 |
| Autenticação | HTTPBasic + secrets.compare_digest  |
...
uvicorn app.main:app --host 0.0.0.0 --port $HTTP_PORT
```
